# README – IT SUPPORT CALL CENTER DATA ANALYSIS USING POWER BI
...
In the initial stage, the problem statement was clearly defined. The objective was to optimize IT support call center team performance by analyzing ticket and call data.

The dataset (1,030 raw records × 22 columns) was explored in detail. Important columns were identified including Ticket_ID, Call_Date, Call_Time, Resolution_Date, Agent_Name, Department, Issue_Type, Priority, Status, Channel, Region, Handle_Time_Minutes, Wait_Time_Seconds, First_Call_Resolution, SLA_Met, Satisfaction_Score, OS_Type, Shift, Escalated, Reopen_Count, Cost_Per_Ticket_USD, and Agent_Experience_Yrs.
...
* Removed 30 duplicate records using Ticket_ID as the key
* Generated 75 missing Ticket IDs sequentially (TKT-XXXX format)
* Standardized 6+ date formats in Call_Date and Resolution_Date to YYYY-MM-DD
* Filled 86 missing Resolution_Dates using Call_Date + median resolution lag
* Fixed Priority typos: 'Hign' → High, 'Mediun' → Medium; standardized casing
* Standardized Status values: 'OPEN', 'resolved', 'Resolve', 'closed' → Open, Resolved, Closed, Escalated, In Progress
* Standardized Channel: 'EMAIL', 'walk in', 'self-service portal' → Email, Walk-In, Self-Service Portal
* Cleaned Yes/No columns (First_Call_Resolution, SLA_Met, Escalated): mapped 'Y', 'n', 'YES', 'NO' → Yes / No / Unknown
* Fixed OS_Type casing: 'WINDOWS 11', 'linux', 'MacOS' → Windows 10, Windows 11, macOS, Linux
* Replaced out-of-range Satisfaction Scores (e.g., -1, 99) with column median (valid range: 1–5)
* Set negative Handle_Time_Minutes and Reopen_Count values to valid defaults
* Filled 280 missing Satisfaction_Score values with median; filled all other numeric nulls with medians
* Filled missing Agent_Name (117), Department (106), Region (189) with 'Unknown'
* Final cleaned dataset: 1,000 rows × 22 columns with zero nulls and zero duplicates
...
* Integration with real-time databases and ticketing systems (e.g., ServiceNow, Jira)
